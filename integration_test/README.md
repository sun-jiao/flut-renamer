...
- The compile and target SDK remain API 37. CI installs pinned command-line tools 23.0, then uses [Android CLI](https://developer.android.com/tools/agents/android-cli#sdk-install) directly: `android --sdk=... sdk install platforms/android-37.0`. The legacy repository ID is `platforms;android-37.0`; the new CLI uses slash-separated IDs and a different display table. `tool/install_android_sdk.sh` retains `integration-sdk-packages.log` for diagnostics but does not parse that human-readable table. Installer failures propagate, and success also requires `platforms/android-37.0/android.jar`. No SDK downgrade is attempted. `node --test tool/install_android_sdk_test.cjs` tests the actual script with fake installers, including failure propagation and the new list format. Emulator API levels are independent of the compile SDK.
...
- Matrix jobs fail independently (no `continue-on-error`); job timeouts are set to 35 minutes for desktop, 45 for Android, and 60 for iOS. `dart_test.yaml` sets the suite-load timeout to 10 minutes because that phase includes a clean native build, installation, launch, and service connection; the CI runner also uses a 10-minute default test timeout, while individual long-running test scenarios still declare their own 2-minute timeout.
